---
title: 'Uttar Pradesh Assembly Election 2017: An Analysis'
author: "Onkar Singh"
date: "`r Sys.Date()`"
output:
  html_document:
...
#   mutate(across(district_name, as.character))
# 
# 
# up_pqt %>% count(district_name)
# glimpse(up_pqt)

# up_2017 <- read_csv("TCPD_AE_Uttar_Pradesh_2021-12-12_2017data.csv") %>% 
#   clean_names()

up <- read_csv("TCPD_AE_Uttar_Pradesh_2021-12-12.csv", col_types = list(
  
  Sub_Region = "c",
  District_Name = "c"
 
)) %>% ## reference column types: https://readr.tidyverse.org/articles/readr.html
  janitor::clean_names() 

up_2017 <- up %>% filter(assembly_no == 17)




up %>%  filter(assembly_no == 17) %>%  count(district_name)
up %>% filter(assembly_no == 17) %>% count(sub_region)
```

```{r results = FALSE}
up_2017 %>%  count(party, sort = T)
...
The data for the above analysis was obtained from the [Election Commission of India portal](https://eci.gov.in/) and [Trivedi Center for Political Data](https://tcpd.ashoka.edu.in/) at the Ashoka University, India.  The various reports and publications referenced below serve as a secondary source in the process of this analysis. The analysis covers each district of the Uttar Pradesh states represented in the map below and provides election results for each of the 294 assembly constituencies in Uttar Pradesh.

![Map of Uttar Pradesh: Districts](map_up_2017_bw.png)

## INTRODUCTION

The Uttar Pradesh Assembly election was one of the most keenly watched Assembly elections in India in 2021. One of the reasons for this interest was an unexpected rise of the Bharatiya Janata Party in a state mostly known for its contests between the Left, Congress, and the Trinamool Congress.

BJP had only 3 seats in the last Assembly election of 2016 whereas the ruling TMC had 212. BJP was never a major player in the state except during the last Parliamentary election where BJP bagged 18 of the 42 parliamentary seats. This raised very high hopes in the BJP camp and the party put all its political might in the Uttar Pradesh Legislative Assembly election concluded in May 2021 where it was pitted against the ruling TMC led by its leader Mamta Banerjee. The stakes were so high that BJP made some of its central ministers resign to contest assembly elections to bolster the party’s prospects in a state traditionally considered home to non-BJP political forces.
...
A total of 303 political parties took part in the Uttar Pradesh assembly election 2017.
...
```{r fig.height=5}
up_2017_total_percent <- up_2017_total_percent%>% 
...
TMC won 213 (73% of total seats), whereas the BJP came to a distant second with 77 (26%) seats, even though it raised its stock significantly in the Uttar Pradesh Assembly from its 2016 tally of a meager 3.

## Total number of seats secured by each political party in each assembly
...
up %>% 
  ...
  filter(position == 1) %>% 
  count(assembly_no, party,  n = "total_seats") %>% 
  mutate(party = fct_reorder(party,n)) %>% 
  ggplot(aes(n, party,
             fill = party)) + 
  geom_col(width = 0.3, show.legend = F) +
...
options(scipen = 100)
 ...
 up_2017_total_percent_dist<- up_2017_total_percent %>% 
  filter (district_name == "BANKURA") 
   
   total_votes_polled <- up_2017_total_percent_dist %>% 
   summarize(district_total_votes = sum(total_votes)) %>% 
   pull(district_total_votes)


largest_votes_dist <- up_2017_total_percent_dist %>%
 slice_max(total_votes, n = 4) %>% 
  filter(row_number() == 1L) %>% 
  pull(total_votes)

second_largest_votes_dist <- up_2017_total_percent_dist %>%
 slice_max(total_votes, n = 4) %>% 
  filter(row_number() == 2L) %>% 
  pull(total_votes)

third_largest_votes_dist <- up_2017_total_percent_dist %>%
 slice_max(total_votes, n = 4) %>% 
  filter(row_number() == 3L) %>% 
  pull(total_votes)

fourth_largest_votes_dist <- up_2017_total_percent_dist %>%
 slice_max(total_votes, n = 4) %>% 
  filter(row_number() == 4L) %>% 
  pull(total_votes)

largest_party_dist <- up_2017_total_percent_dist %>%
 slice_max(total_votes, n = 4) %>% 
  filter(row_number() == 1L) %>% 
  pull(party)

second_largest_party_dist <- up_2017_total_percent_dist %>%
 slice_max(total_votes, n = 4) %>% 
  filter(row_number() == 2L) %>% 
  pull(party)

third_largest_party_dist <- up_2017_total_percent_dist %>%
 slice_max(total_votes, n = 4) %>% 
  filter(row_number() == 3L) %>% 
  pull(party)

fourth_largest_party_dist <- up_2017_total_percent_dist %>%
 slice_max(total_votes, n = 4) %>% 
  filter(row_number() == 4L) %>% 
  pull(party)
...
largest_percent_dist <- up_2017_total_percent_dist %>%
 slice_max(percent_votes, n = 4) %>% 
  filter(row_number() == 1L) %>% 
  pull(percent_votes)

second_largest_percent_dist <- up_2017_total_percent_dist %>%
 slice_max(percent_votes, n = 4) %>% 
  filter(row_number() == 2L) %>% 
  pull(percent_votes)


third_largest_percent_dist <- up_2017_total_percent_dist %>%
slice_max(percent_votes, n = 4) %>% 
  filter(row_number() == 3L) %>% 
  pull(percent_votes)

fourth_largest_percent_dist <- up_2017_total_percent_dist %>%
 slice_max(percent_votes, n = 4) %>% 
  filter(row_number() == 4L) %>% 
  pull(percent_votes)
